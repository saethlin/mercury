...
!
! texadactyl_20180507.1 Handle warnings in mio_err() calls about character string lengths
! texadactyl_20180507.2 Handle warnings in mio_spl() calls about argument array bounds too small
! texadactyl_20180507.3 nnn open(..., err=nnn) is an infinite loop
!                       If one cannot open a file for output, let the program fail.
! texadactyl_20180514.1 Once CMAX is exceeded by variable the nclo counter,&
!                       this will cause memory out of bounds errors.
! texadactyl_20180514.2 Return a nonzero error code to the O/S from mio_err().
!                       Repeat info.out diagnostic on the operator console.
!
!------------------------------------------------------------------------------
!
! Variables:
...
!  ID     = name of the object (25 characters)
...
! If objects had close encounters, advance H_K using Bulirsch-Stoer instead
      if (nce.gt.0) then
        ...
        call mdt_hkce (time,tstart,h0,hrec,tol,rmax,en(3),jcen,rcen,&
          ...
          ngflag,colflag,ce,nce,ice,jce,nclo,iclo,jclo,dclo,tclo,&
          ixvclo,jxvclo,outfile,mem,lmem,mfo_hkce)
      end if
!
! Advance solar Hamiltonian for H/2
      mvsum(1) = 0.d0
...
! Read values of mass, close-encounter limit, density etc.
        do k = 3, nsub, 2
          ...
          if (c80(1:1).eq.'m'.or.c80(1:1).eq.'M') then
            ...
            then
            call mio_err (23,mem(81),lmem(81),mem(97),lmem(97),&
